...
Game.inputHints = {
    keyboard = {select="Enter", back="ESC", navigate="Arrow Keys", skip="SPACE", confirm="Enter", cancel="ESC", action="Space"},
    gamepad  = {select="A", back="B", navigate="D-Pad", skip="A", confirm="A", cancel="B", action="X"}
}
...
local function initWindow()
    ...
    lw.setMode(800, 600, {fullscreen=false, resizable=true, minwidth=constants.window.minWidth, minheight=constants.window.minHeight})
...
local function initStates()
    ...
    if CONFIG.debug then
        ...
        debugConsole = {update=function() end, draw=function() end, keypressed=function() return false end, textinput=function() return false end}
    end
...
local function loadSettings()
    ...
    if lines[6] then Game.selectedShip = lines[6] end
    Game.highContrast      = lines[7]=="true"
    if lines[8] then Game.fontScale=tonumber(lines[8]) or 1 end
    updateAudioVolumes() ; applyFontScale()
end
...
local function applyWindowMode()
    local res = {
        {800,600}, {1024,768}, {1280,720}, {1366,768}, {1920,1080}, {2560,1440}
    }
    local flags = {vsync=1, minwidth=constants.window.minWidth, minheight=constants.window.minHeight}
    if Game.displayMode=="borderless" then
        flags.fullscreen=true; flags.fullscreentype="desktop"; flags.resizable=false
        lw.setMode(0,0,flags)
    elseif Game.displayMode=="fullscreen" then
        local r=res[Game.currentResolution] or res[1]
        flags.fullscreen=true; flags.fullscreentype="exclusive"; flags.resizable=false
        lw.setMode(r[1], r[2], flags)
    else
        local r=res[Game.currentResolution] or res[1]
        flags.fullscreen=false; flags.resizable=true
        lw.setMode(r[1], r[2], flags)
    end
    initStarfield()
end
...
function playPositionalSound(src,x,y)
    ...
    if src==Game.laserSound and Game.laserClones then
        ...
        c=src:clone(); c.baseVolume=src.baseVolume
    end
    local dx,dy = x-player.x, y-player.y
    if c:getChannelCount()==1 then
        c:setRelative(true); c:setPosition(dx,dy,0)
        c:setAttenuationDistances(soundReferenceDistance, soundMaxDistance)
    end
    c:setVolume((c.baseVolume or 1)*Game.sfxVolume*Game.masterVolume); c:play()
end
...
function initStarfield()
    ...
    for i=1,starCount do
        stars[#stars+1]={x=love.math.random()*lg.getWidth(), y=love.math.random()*lg.getHeight(), speed=love.math.random()*50+20, size=love.math.random()*2}
    end
end
function updateStarfield(dt)
    ...
    for _,s in ipairs(stars) do
        s.y=s.y+s.speed*dt
        if s.y>h then s.y=-s.size; s.x=love.math.random()*lg.getWidth() end
    end
end
function drawStarfield()
    for _,s in ipairs(stars) do
        local b=s.size/2; lg.setColor(b,b,b); lg.circle("fill",s.x,s.y,s.size)
    end
end
...
-- ---------------------------------------------------------------------------
function love.load()
    initWindow() ; loadFonts() ; loadAudio() ; loadSettings() ; applyWindowMode()
    ...
    Game.availableShips, Game.selectedShip, Game.spriteScale = {"alpha","beta","gamma"}, "alpha", 0.15
    initStates()
...
function love.update(dt)
    dt=math.min(dt,1/30); if _G.timeScale then dt=dt*_G.timeScale end
    updateStarfield(dt); debugConsole:update(dt)
    if _G.configReloadNotification then
        local n=_G.configReloadNotification; n.timer=n.timer-dt; if n.timer<=0 then _G.configReloadNotification=nil end
    end
...
function love.draw()
    ...
    if _G.configReloadNotification then
        local n=_G.configReloadNotification; lg.setFont(Game.menuFont)
        lg.setColor(n.color[1],n.color[2],n.color[3],n.timer); lg.printf(n.text,0,lg.getHeight()/2-50,lg.getWidth(),"center"); lg.setColor(1,1,1,1)
    end
    if _G.showLogOverlay then logger.drawOverlay(10,lg.getHeight()-200,10) end
    debugConsole:draw()
end

function love.keypressed(k,sc,isr)
    updateInputType("keyboard")
    if debugConsole:keypressed(k) then return end
    if k=="f3" then debugMode=not debugMode; logger.info("Debug mode: %s",debugMode and "on" or "off")
    elseif k=="f5" then
        local ok,newC=pcall(function() package.loaded["src.constants"]=nil; return require("src.constants") end)
        if ok then constants=newC; logger.info("Config reloaded"); _G.configReloadNotification={text="Config Reloaded!",timer=2,color={0,1,0}}
        else logger.error("Reload failed: "..tostring(newC)); _G.configReloadNotification={text="Reload Failed!",timer=2,color={1,0,0}} end
    elseif k=="f9" then debugOverlay=not debugOverlay end
    stateManager:keypressed(k,sc,isr)
end

function love.keyreleased(k,sc) stateManager:keyreleased(k,sc) end
function love.mousepressed(x,y,b,ist,p) updateInputType("keyboard"); stateManager:mousepressed(x,y,b,ist,p) end
function love.mousemoved() updateInputType("keyboard") end
function love.gamepadaxis(_,_,v) if math.abs(v)>0.2 then updateInputType("gamepad") end end
function love.gamepadpressed(js,b) updateInputType("gamepad"); stateManager:gamepadpressed(js,b) end
function love.gamepadreleased(js,b) stateManager:gamepadreleased(js,b) end
function love.resize(w,h) initStarfield(); stateManager:resize(w,h); logger.info("Resized to %dx%d",w,h) end
function love.textinput(t) debugConsole:textinput(t) end

-- Debug helpers -------------------------------------------------------------
debugMode, debugOverlay, frameTimeHistory, maxFrameTimeHistory = false,false,{},60
local function drawDebugInfo()
    lg.setFont(Game.smallFont); lg.setColor(1,1,1,0.8)
    table.insert(frameTimeHistory, lt.getDelta()); if #frameTimeHistory>maxFrameTimeHistory then table.remove(frameTimeHistory,1) end
    local avg,max=0,0; for _,d in ipairs(frameTimeHistory) do avg=avg+d; if d>max then max=d end end; avg=avg/#frameTimeHistory
    local t={"FPS: "..lt.getFPS(),string.format("Mem: %.2f MB",collectgarbage("count")/1024),
             string.format("Delta: %.3f ms",lt.getAverageDelta()*1000),
             string.format("Avg: %.3f ms",avg*1000),string.format("Max: %.3f ms",max*1000),
             "State: "..(stateManager.currentName or "none"),
             string.format("Res: %dx%d",lg.getWidth(),lg.getHeight())}
    for i,l in ipairs(t) do lg.print(l,lg.getWidth()-150,10+15*(i-1)) end
end
local function drawDebugOverlay()
    lg.setFont(Game.smallFont); lg.setColor(0,0,0,0.7); lg.rectangle("fill",lg.getWidth()-250,10,240,180,5)
    lg.setColor(1,1,1,1); local x,y,lh=lg.getWidth()-240,20,18
    lg.print(string.format("Mem: %.2f MB",collectgarbage("count")/1024),x,y); y=y+lh
    ...
    y=y+lh; lg.setColor(0.7,0.7,0.7,1)
...
function checkCollision(a,b)
    local aw,ah,bw,bh=a.width or a.size,a.height or a.size,b.width or b.size,b.height or b.size
    return (a.x-aw/2)<(b.x+bw/2) and (a.x+aw/2)>(b.x-bw/2) and (a.y-ah/2)<(b.y+bh/2) and (a.y+ah/2)>(b.y-bh/2)
end
function love.quit()
    if Game.spriteManager and Game.spriteManager.reportUsage then Game.spriteManager:reportUsage() end
end
