-- File: main.lua
------------------------------------------------------------------------------
-- Stellar Assault – Main Module (conflict‑resolved)
--   * Adds package.path entries so Lua can locate third‑party libs inside src/
--   * Integrates Mixer‑based sound management
--   * Removes every <<<<<<< / ======= / >>>>>>> merge marker
------------------------------------------------------------------------------
...
local Mixer        = require("src.Mixer")          -- NEW
...
Game.debugConsole   = nil     -- retained for callbacks
debugConsole        = nil     -- legacy global
...
Game.audioPool = nil
Game.mixer     = Mixer.new()                 -- central audio router
...
local function loadAudio()
  ...
  if Game.menuSelectSound then
    ...
    Game.mixer:register(Game.menuConfirmSound, "sfx")
  end
  ...
  -- Pool
  Game.audioPool = AudioPool:new(8)
...
function playPositionalSound(src, x, y)
  if not src or not player or not Game.audioPool then return end

  local name
  if     src == Game.laserSound       then name = "laser"
  elseif src == Game.explosionSound   then name = "explosion"
  elseif src == Game.powerupSound     then name = "powerup"
  elseif src == Game.shieldBreakSound then name = "shield_break"
  elseif src == Game.gameOverSound    then name = "gameover"
  elseif src == Game.menuSelectSound  then name = "menu_select"
  elseif src == Game.menuConfirmSound then name = "menu_confirm"
  elseif src == Game.victorySound     then name = "victory"
  end
...
function initStarfield() starfield = Starfield.new(200) end
function updateStarfield(dt) if starfield then starfield:update(dt) end end
function drawStarfield() if starfield then starfield:draw() end end

_G.initStarfield   = initStarfield
_G.updateStarfield = updateStarfield
_G.drawStarfield   = drawStarfield

-- expose init helpers for hot‑reload
...
function love.load()
  ...
  -- Ship & enemy sprites (paths use new asset layout)
  Game.playerShips = {
...
function love.update(dt)
  ...
  while accumulator >= FIXED_DT do
    ...
    if _G.configReloadNotification then
      ...
      if n.timer <= 0 then _G.configReloadNotification = nil end
    end
  ...
  elseif k == "f5" then
    ...
    if ok then
      ...
      _G.configReloadNotification = {
        text = "Config Reloaded!", timer = 2, color = {0, 1, 0}
      }
    else
      logger.error("Reload failed: %s", tostring(newC))
      _G.configReloadNotification = {
        text = "Reload Failed!", timer = 2, color = {1, 0, 0}
      }
    end
  elseif k == "f9" then
...
function drawDebugInfo()
  ...
  if #frameTimeHistory > maxFrameTimeHistory then table.remove(frameTimeHistory, 1) end
...
function love.run(...)
  local _, result = xpcall(_original_love_run, _handle_error, ...)
  return result
end
