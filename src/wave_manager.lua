...
function WaveManager:startWave(waveNumber)
  ...
  logger.info(
    "Starting wave " .. self.waveNumber .. " with " .. self.remainingToSpawn .. " enemies"
  )
end
...
function WaveManager:spawnEnemy()
  ...
  enemy.health = math.ceil(
    (enemyType.health + math.floor(self.waveNumber / 5))
      * self.difficultyMultiplier
      * (self.waveDifficulty or 1)
  )
...
function WaveManager:updateLasers(dt)
  ...
  for i = #self.enemyLasers, 1, -1 do
    ...
    if
      laser.y > love.graphics.getHeight() + laser.height
      or laser.x < -laser.width
      or laser.x > love.graphics.getWidth() + laser.width
    then
