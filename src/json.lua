...
function json.encode(value, stack)
  ...
  elseif t == "string"  then return encode_string(value)
  elseif t == "number"  then return encode_number(value)
  elseif t == "boolean" then return tostring(value)
  elseif t == "table"   then return encode_table(value, stack or {})
  else
...
decode_literal = function(str, idx, lit, val)
  ...
  if i ~= idx then decode_error(str, idx, "expected '" .. lit .. "'") end
  return val, j + 1
end
...
local function decode_number(str, idx)
  ...
  if not num then decode_error(str, idx, "invalid number") end
...
local function decode_string(str, idx)
  ...
  while true do
    ...
    if c == '"' then return _concat(res), idx + 1 end
    if c == "\\" then
      ...
      if nextc == "u" then
        ...
        _insert(res, utf8 and utf8.char(tonumber(hex, 16)) or
        ...
        if not repl then decode_error(str, idx, "invalid escape char") end
        _insert(res, repl)
        idx = idx + 2
      end
    else
...
decode_value = function(str, idx)
  idx = skip_ws(str, idx)
  local c = str:sub(idx, idx)
  if     c == "{" then return decode_object(str, idx)
  elseif c == "[" then return decode_array(str, idx)
  elseif c == '"' then return decode_string(str, idx)
  elseif c == "-" or c:match("%d") then return decode_number(str, idx)
  elseif c == "t" then return decode_literal(str, idx, "true",  true)
  elseif c == "f" then return decode_literal(str, idx, "false", false)
  elseif c == "n" then return decode_literal(str, idx, "null",  nil)
  else
