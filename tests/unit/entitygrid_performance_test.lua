...
local function createEntities(count)
    local list = {}
    for i=1,count do
        table.insert(list,{x=math.random(0,800),y=math.random(0,600),width=20,height=20,tag='asteroid'})
    end
    return list
end
...
describe("SpatialHash performance", function()
    it("is faster than naive search", function()
        ...
        for _,a in ipairs(asteroids) do grid:insert(a) end

        local function naive()
            local c=0
            for _,a in ipairs(asteroids) do
                for _,l in ipairs(lasers) do
                    if Collision.checkAABB(a,l) then c=c+1 end
                end
            end
            return c
        end
        ...
        local function hashed()
            ...
            for _,l in ipairs(lasers) do
                for _,e in ipairs(grid:getNearby(l)) do
                    if e.tag=='asteroid' and Collision.checkAABB(e,l) then c=c+1 end
                end
            end
            return c
        end
