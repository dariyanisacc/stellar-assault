...
function LevelSelectState:update(dt)
    ...
    -- Shop button hover
    self.shopButton.hover = mx >= self.shopButton.x and mx <= self.shopButton.x + self.shopButton.width and
                           my >= self.shopButton.y and my <= self.shopButton.y + self.shopButton.height
    
    -- Ship select button hover
    self.shipSelectButton.hover = mx >= self.shipSelectButton.x and mx <= self.shipSelectButton.x + self.shipSelectButton.width and
                                  my >= self.shipSelectButton.y and my <= self.shipSelectButton.y + self.shipSelectButton.height
    ...
    if not self.shopOpen and not self.shipSelectOpen then
        for _, button in ipairs(self.levelButtons) do
            button.hover = button.unlocked and 
    ...
    for i, shipName in ipairs(availableShips) do
        ...
        if isUnlocked then
            ...
            if playerShips and playerShips[shipName] then
                ...
                lg.setColor(1, 1, 1)
                lg.draw(sprite, x + width - 60, shipY + 20, 0, scale, scale,
...
function LevelSelectState:keypressed(key)
    ...
    elseif self.shopOpen and self.shopManager then
        ...
        elseif key == "return" or key == "space" then
            -- Try to purchase
            local success, newScore, message = self.shopManager:tryPurchase(
                self.shopManager.selected, 
                self.currentScore
            )
