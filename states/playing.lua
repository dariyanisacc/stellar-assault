...
function PlayingState:updateLasers(dt)
  ...
  for i = #self.scene.alienLasers, 1, -1 do
    ...
    if
      laser.y > self.screenHeight + laser.height
      or laser.y < -laser.height
      or laser.x < -laser.width
...
function PlayingState:handleShieldHit(entity, index, array)
  ...
  if Game and Game.audioPool then
    Game.audioPool:play("shield_break", entity.x, entity.y)
  elseif shieldBreakSound and shieldBreakSound.play then
...
function PlayingState:drawUI()
  ...
  if self.waveManager then
    ...
    local enemyCount = (self.waveManager.getEnemyCount and self.waveManager:getEnemyCount())
      or #self.waveManager.enemies
  ...
  if activePowerupCount > 0 then
    ...
    for powerup, timer in pairs(self.scene.activePowerups) do
      ...
      if powerupY > powerupPanelY + powerupPanelHeight - 5 then
        break
  ...
  if self.showControlsHint then
    ...
    lg.print(
      "MOVE: Arrows/"
        .. (self.keyBindings.up or "W"):upper()
        .. (self.keyBindings.left or "A"):upper()
        .. (self.keyBindings.down or "S"):upper()
        .. (self.keyBindings.right or "D"):upper(),
      controlsX,
      controlsY
    )
    ...
    lg.print(
      "BOMB: "
        .. (self.keyBindings.bomb or "B"):upper()
        .. " | BOOST: "
        .. (self.keyBindings.boost or "Shift"):upper(),
      controlsX,
      controlsY + 24
    )
...
function PlayingState:spawnBoss()
  if bossSpawned or self.bossManager.activeBoss then
    return
  end
...
function PlayingState:updateBoss(dt)
  if not self.bossManager.activeBoss then
    return
  end
