...
function OptionsState:keypressed(key)
    ...
    if key == "up" then
        ...
        if self.selection < 1 then self.selection = #self.menuItems end
        if menuSelectSound then menuSelectSound:play() end
    elseif key == "down" then
        self.selection = self.selection + 1
        if self.selection > #self.menuItems then self.selection = 1 end
        if menuSelectSound then menuSelectSound:play() end
...
function OptionsState:controlsKeypressed(key)
    ...
    if key == "up" then
        ...
        until self.controlsMenuItems[self.controlsSelection].type ~= "spacer" and 
              self.controlsMenuItems[self.controlsSelection].type ~= "header"
        if menuSelectSound then menuSelectSound:play() end
    elseif key == "down" then
        repeat
            self.controlsSelection = self.controlsSelection + 1
