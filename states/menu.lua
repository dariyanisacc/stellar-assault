...
function MenuState:update(dt)
    ...
    if self.loadErrorTimer and self.loadErrorTimer > 0 then
        ...
        if self.loadErrorTimer <= 0 then self.loadErrorMessage = nil end
    end
    ...
    if js and js:isGamepad() then
        ...
        if math.abs(jy) > 0.5 then
            if jy < 0 then self:_analogPress("up",    dt) end
            if jy > 0 then self:_analogPress("down",  dt) end
        else
        ...
        if math.abs(jx) > 0.5 then
            if jx < 0 then self:_analogPress("left",  dt) end
            if jx > 0 then self:_analogPress("right", dt) end
        else
...
function MenuState:draw()
    ...
    -- Active sub-menu
    if     self.menuState == "main"        then self:drawMainMenu()
    elseif self.menuState == "saves"       then self:drawSaveMenu()
    elseif self.menuState == "levelselect" then self:drawLevelSelect()
    elseif self.menuState == "shipselect"  then self:drawShipSelect()
    end
...
function MenuState:drawMainMenu()
    ...
    local options = {
        "Start Game", "Level Select", "Select Ship",
        "Leaderboard", "Options", "Quit"
    }

    for i, option in ipairs(options) do
        if i == self.selection then
            lg.setColor(Game.highContrast and 1 or 1,
    ...
    Game.uiManager:drawMessage(msg, self.screenWidth/2,
                               self.screenHeight-30, colour, Game.smallFont)
end
...
function MenuState:drawSaveMenu()
    ...
    for i = 1, 3 do
        lg.setColor(i == self.selectedSaveSlot and 1 or 1,
        ...
        lg.print(text, self.screenWidth/2 - lg.getFont():getWidth(text)/2,
                        250 + i*50)
    end

    -- Back option (mapped to faux "slot 4")
    lg.setColor(self.selectedSaveSlot == 4 and 1 or 1,
...
function MenuState:drawLevelSelect()
    ...
    for r = 0, rows-1 do
        for c = 0, cols-1 do
            ...
            if lvl <= 15 then
                ...
                if lvl == self.selectedLevel then      lg.setColor(1,1,0)
                elseif unlocked then                   lg.setColor(0,1,1)
                else                                   lg.setColor(0.3,0.3,0.3)
                end
                ...
                lg.print(s, x+box/2 - lg.getFont():getWidth(s)/2,
    ...
    lg.setFont(Game.menuFont)
    lg.setColor(self.selectedLevel==16 and 1 or 1,
...
function MenuState:drawShipSelect()
    ...
    for i, name in ipairs(Game.availableShips) do
        local x = self.screenWidth/2 - (#Game.availableShips*gap)/2
                  + (i-1)*gap + gap/2
        ...
        if sprite then
            ...
            lg.setColor(1,1,1)
            lg.draw(sprite, x, shipY, 0, scale, scale,
                    sprite:getWidth()/2, sprite:getHeight()/2)
        else
            lg.setColor(i==self.selectedShipIndex and 1 or 0.7,
        ...
        -- Name
        lg.setFont(Game.smallFont)
        lg.setColor(i==self.selectedShipIndex and 1 or 1,
    ...
    Game.uiManager:drawMessage(instr, self.screenWidth/2,
                               self.screenHeight-30, {0.7,0.7,0.7}, Game.smallFont)
end
...
function MenuState:keypressed(key)
    ...
    elseif self.menuState == "saves"       then self:handleSaveMenuInput(key)
    elseif self.menuState == "levelselect" then self:handleLevelSelectInput(key)
    elseif self.menuState == "shipselect"  then self:handleShipSelectInput(key)
    end
end
...
function MenuState:gamepadpressed(_, button)
    local map = {
        ...
        a="return", start="return", b="escape"
    }
...
function MenuState:handleMainMenuInput(key)
    ...
    elseif key=="return" or key=="space" then
        ...
        if     self.selection==1 then self.menuState="saves"
        elseif self.selection==2 then
            self.menuState="levelselect"; self.selectedLevel=1
            self.levelSelectSource="main"; currentSaveSlot=1
        elseif self.selection==3 then
            self.menuState="shipselect"; self.selectedShipIndex=1
            for i,ship in ipairs(Game.availableShips) do
                if ship==Game.selectedShip then self.selectedShipIndex=i break end
            end
        elseif self.selection==4 then stateManager:switch("leaderboard","menu")
        elseif self.selection==5 then stateManager:switch("options")
        elseif self.selection==6 then love.event.quit()
        end
    end
end
    ...
    elseif key=="return" or key=="space" then
        ...
        if self.selectedSaveSlot==4 then
            ...
            if self.saveSlots[currentSaveSlot] then
                ...
                currentLevel = 1
                if Game.backgroundMusic then Game.backgroundMusic:stop() end
...
function MenuState:handleLevelSelectInput(key)
    ...
    if key=="left"  then self.selectedLevel = math.max(1,                self.selectedLevel-1)
    elseif key=="right" then
        self.selectedLevel = (self.selectedLevel<maxLvl) and
                              (self.selectedLevel+1) or 16
    elseif key=="up" then
        if self.selectedLevel>cols and self.selectedLevel<=maxLvl then
            self.selectedLevel = self.selectedLevel - cols
        elseif self.selectedLevel==16 then self.selectedLevel=maxLvl end
    elseif key=="down" then
    ...
    elseif key=="return" or key=="space" then
        ...
        if self.selectedLevel==16 then
            ...
            local unlocked = (self.levelSelectSource=="main") or
                              (self.saveSlots[currentSaveSlot] and
                               self.selectedLevel<=self.saveSlots[currentSaveSlot].level) or
                              self.selectedLevel==1
            if unlocked then
